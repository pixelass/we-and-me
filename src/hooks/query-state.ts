...
export const useQueryStates = <T>(
	...
	const state = useMemo(
		() =>
			Object.entries(pick(values, Object.keys(defaultValues))).reduce<Partial<T>>(
				(previousValue: Partial<T>, [key, value]) => {
					return {
						...previousValue,
						[key]: parse[key as keyof T]
							? parse[key as keyof T](value as string)
							: value,
					};
				},
				{}
	...
	const setState: QueryStateDispatcher<Partial<T>> = useCallback(
		async (newState, history_) => {
			...
			const encoded = Object.entries(newState).reduce((previousValue, [key, value]) => {
				const updated: Partial<T> = {
					...previousValue,
				};
				if (value) {
					updated[key] = serialize[key as keyof T]
